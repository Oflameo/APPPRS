...
bool WaypointsGoalNode::init()
{
  ...
  pnh.param("robot_frame",    robot_frame_,    std::string("/base_link"));
  ...
  ROS_INFO("waypoints_navi initialized");
...
void WaypointsGoalNode::navCtrlCB(const yocs_msgs::NavigationControl::ConstPtr& nav_ctrl)
{
  if (nav_ctrl->control == yocs_msgs::NavigationControl::STOP)
  {
    ...
    {
      ROS_INFO_STREAM("Stopping current trajectory execution ...");
      cancelAllGoals();
      resetWaypoints();
      ROS_INFO_STREAM("Current trajectory execution stopped.");
      idle_status_update_sent_ = false;
      state_ = IDLE;
  ...
  {
	ROS_INFO("Starting current trajectory execution ...");

	if ((state_ == PAUSED))
	{
		ROS_INFO_STREAM("Resuming current trajectory execution ...");
        state_ = START;
        mode_  = GOAL;

	}
	else if ((state_ == IDLE) || (state_ == COMPLETED))
    {
      resetWaypoints();
  ...
  else if ((nav_ctrl->control == yocs_msgs::NavigationControl::PAUSE))
  {
	if ((state_ == START) || (state_ == ACTIVE))
	{
	  ROS_INFO_STREAM("Pausing current execution ...");
	  cancelAllGoals();
	  state_ = PAUSED;
	  publishStatusUpdate(yocs_msgs::NavigationControlStatus::PAUSED);
	}
	else
	{
	  ROS_WARN_STREAM("Cannot pause way point/trajectory execution, because nothing is being executed.");
	}
  }
}
    ...
    else if (state_ == ACTIVE)
    {
          ...
          (goal_state == actionlib::SimpleClientGoalState::PREEMPTED))
      {
        ...
        if (waypoints_it_ < (waypoints_.end() - 1))
        {
          ...
          // ROS_INFO("Distance to current goal (%.2f, %.2f m).", distance, close_enough_);
