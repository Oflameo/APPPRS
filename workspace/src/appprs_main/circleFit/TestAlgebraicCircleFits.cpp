...
//    cv::Mat Image=cv::Mat(15,15, CV_64F, cvScalar(0.));
uint16_t rows=150;
uint16_t cols=150;

   cv::Mat Image =cv::Mat::zeros(rows,cols,CV_8U);

    int j;
    int k;
    for(int i=0; i<data1.n; i++)
    {
    	j=BenchmarkExampleDataX[i];
    	k=BenchmarkExampleDataY[i];
    	Image.at<cv::Vec2d>(j*10,k*10)=255;
    }

   imshow("example 1", Image);
   cv::waitKey(0);
/*
...
//         << circle.r << "  sigma " << circle.s << endl;
    ...
    cv::Mat Image2 =cv::Mat::zeros(rows,cols,CV_8U);


       for(int i=0; i<data1.n; i++)
       {
       	j=floor(150*(data2.X[i]+1));
       	k=floor(150*(data2.Y[i]+1));
       	Image2.at<cv::Vec2d>(j,k)=255;
       	std::cout<<"x="<<j<<" y="<<k<<std::endl;
       }

       data2.print();

      imshow("example 2", Image2);
      cv::waitKey(0);
