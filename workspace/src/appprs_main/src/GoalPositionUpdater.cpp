...
GoalPositionUpdater::GoalPositionUpdater() :
		goals_received_(0), current_global_waypoint_index_(0), current_local_waypoint_index_(
				1), last_joy_msg_(sensor_msgs::Joy()) {
	...
	goal_publisher_ = nh_.advertise<geometry_msgs::PoseStamped>(
			"/move_base_simple/goal2", 10);
	steer_command_publisher_ = nh_.advertise<std_msgs::Float32>("/path_steer",
			1);
	speed_command_publisher_ = nh_.advertise<std_msgs::Float32>("/path_speed",
			1);
	car_heading_pub_=nh_.advertise<std_msgs::Float32>("/car_heading",1);
	waypoint_heading_pub_=nh_.advertise<std_msgs::Float32>("/waypoint_heading", 1);

	goal_subscriber_ = nh_.subscribe<geometry_msgs::PoseStamped>(
			"/move_base_simple/goal", 10, &GoalPositionUpdater::goal_callback,
			this);
	joy_subscriber_ = nh_.subscribe<sensor_msgs::Joy>("/joy", 10,
			&GoalPositionUpdater::joy_callback, this);
	timer_ = nh_.createTimer(ros::Duration(0.1),
			&GoalPositionUpdater::timer_callback, this);
	ros::NodeHandle pnh("~");
	std::string waypoint_file_name;
	pnh.param("waypoint_file", waypoint_file_name,
			std::string("/tmp/waypoints.txt"));
	distance_threshold_ = 1.0;

	ROS_INFO_STREAM(
			"Attempting to open waypoint file " << waypoint_file_name << ".");
	...
	if (waypointFile.is_open()) {
		while (std::getline(waypointFile, line)) {
			...
			if (number_list.size() < 7) {
				ROS_FATAL_STREAM(
						"Unable to parse way point list, less than 7 tokens in line "<< line << "!");
				continue;
			}
		...
		waypointFile.close();
		ROS_INFO_STREAM(
				"Way point file with " << global_waypoint_list_.size() << " way points has been successfully parsed.");
...
void GoalPositionUpdater::goal_callback(
		const geometry_msgs::PoseStamped::ConstPtr& msg) {
	last_goal_ = current_goal_;
	current_goal_ = Eigen::Vector3d(msg->pose.position.x, msg->pose.position.y,
			msg->pose.position.z);
	++goals_received_;
...
void GoalPositionUpdater::timer_callback(const ros::TimerEvent& e) {
	...
	geometry_msgs::PoseStamped pose_msg =
			local_waypoint_list_[current_local_waypoint_index_];
	tf::Transform transform_auxiliar;
	transform_auxiliar.setOrigin(
			tf::Vector3(pose_msg.pose.position.x, pose_msg.pose.position.y,
					pose_msg.pose.position.z));
	transform_auxiliar.setRotation(
			tf::Quaternion(pose_msg.pose.orientation.x,
					pose_msg.pose.orientation.y, pose_msg.pose.orientation.z,
					pose_msg.pose.orientation.w));
	tf_broadcaster_.sendTransform(
			tf::StampedTransform(transform_auxiliar, ros::Time::now(), "/map",
					"/waypoint"));
	...
	if (result >= 1) { //advance to next local  waypoint
		...
		if (current_local_waypoint_index_ >= local_waypoint_list_.size() - 2) { //advance to next global waypoint
			...
			if (current_global_waypoint_index_
					>= global_waypoint_list_.size()) {
	...
	next_waypoint.pose =
			local_waypoint_list_[current_local_waypoint_index_].pose;
...
int GoalPositionUpdater::recomputeLocalPath(bool use_last_pose,
	...
	if (use_last_pose) {
		robot_pose = Eigen::Vector3d(last_pose.pose.position.x,
				last_pose.pose.position.y, last_pose.pose.position.z);
		robot_quat = Eigen::Quaterniond(last_pose.pose.orientation.w,
				last_pose.pose.orientation.x, last_pose.pose.orientation.y,
				last_pose.pose.orientation.z);

	} else {
		robot_pose = Eigen::Vector3d(transform_w_b.getOrigin().x(),
				transform_w_b.getOrigin().y(), transform_w_b.getOrigin().z());
		robot_quat = Eigen::Quaterniond(transform_w_b.getRotation().getW(),
				transform_w_b.getRotation().getX(),
				transform_w_b.getRotation().getY(),
				transform_w_b.getRotation().getZ());
	}

	//std::cout << "robot: " << robot_quat.toRotationMatrix().eulerAngles(0, 1, 2)*180/M_PI << ", " << acos(robot_quat.w())*2.0*180/M_PI << std::endl;
	Eigen::Quaterniond waypoint_quat(
			global_waypoint_list_[current_global_waypoint_index_].pose.orientation.w,
	...
	local_waypoint_list_ =
					getPath(robot_pose(0), robot_pose(1),
...
int GoalPositionUpdater::checkPosition() {
	...
	try {
		tf_listener_.lookupTransform("/waypoint", "/base_link", ros::Time(0),
				transform_r_base);
	} catch (tf::TransformException& ex) {
		return 0;
	}
	Eigen::Vector3d base_pose = Eigen::Vector3d(
			transform_r_base.getOrigin().x(), transform_r_base.getOrigin().y(),
			transform_r_base.getOrigin().z());
...
void GoalPositionUpdater::computeAndPublishNextCommand() {
	...
	tf::StampedTransform transform_car;
	tf::StampedTransform transform_local_way;
	try {
		tf_listener_.lookupTransform("/base_link", "/waypoint", ros::Time(0),
				transform_r_way);
		tf_listener_.lookupTransform("/map", "/base_link", ros::Time(0), transform_car);
		tf_listener_.lookupTransform("/map", "/waypoint", ros::Time(0), transform_local_way);
	} catch (tf::TransformException& ex) {
		return;
	}
	//float waypoint_angle = transform_r_way.getOrigin().w();
	Eigen::Vector3d waypoint_pose = Eigen::Vector3d(
			transform_r_way.getOrigin().x(), transform_r_way.getOrigin().y(),
			transform_r_way.getOrigin().z());
	//std::cout << waypoint_pose << std::endl;
	float theta = atan2(waypoint_pose[1], waypoint_pose[0]);


	std_msgs::Float32 steer_cmd_msg, speed_cmd_msg, car_theta, waypoint_theta;
	car_theta.data=transform_car.getRotation().getAngle()*180.0/M_PI;
	waypoint_theta.data=transform_local_way.getRotation().getAngle()*180.0/M_PI;
	//ROS_INFO("Car_theta =%f, waypoint_theta=%f",car_theta.data, waypoint_theta.data); //print results

	steer_cmd_msg.data = std::min(std::max(-theta * 180 / M_PI, -20.0), 20.0);
	speed_cmd_msg.data = (40.0 - fabs(steer_cmd_msg.data)) * (1 / 40.0); //between 0.125 at 35deg and 1 at 0 deg

	ROS_DEBUG_STREAM(
			"Send angle: " << steer_cmd_msg.data << ", velocity: " << speed_cmd_msg.data);
	car_heading_pub_.publish(car_theta); //publish car angle in world frame
	waypoint_heading_pub_.publish(waypoint_theta); //publish waypoint angle in world frame
	steer_command_publisher_.publish(steer_cmd_msg); //publish message
	speed_command_publisher_.publish(speed_cmd_msg); //publish messag
